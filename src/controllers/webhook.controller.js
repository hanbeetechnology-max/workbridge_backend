...
import * as webhookEventsRepo from "../repositories/CashFree_webhook_events.repository.js";
...
// CashFree_webhook_events' own dedupe: only a PENDING_FUNDS project /
...
async function handlePaymentFailed(payload) {
  ...
  const result = await transaction(async (client) => {
    const project = await projectsRepo.findByCashFreeOrderId(client, payment.order_id);
...
// REFUND ledger row and CashFree_refund_id synchronously, at the moment
// they *call* createRefund — this event is CashFree's own confirmation
// that the refund settled, and needs no further write on WorkBridge's
// side. The event itself is still durably recorded by CashFree_webhook_
...
// event names in CashFree's current webhook catalog (no generic
// "account.activated"/"account.rejected" exist there). Keeps a worker's
// CashFree_account_status current without WorkBridge ever polling
// CashFree for it; completeProject's Route-transfer eligibility check
...
// all — REJECTED (still a valid CashFree_account_status value) is set
// manually by staff if CashFree's own dashboard shows a failed account,
...
async function handleAccountStatusEvent(payload, status) {
  ...
  if (!accountId) return;
  await usersRepo.updateCashFreeAccountStatusByAccountId(accountId, status);
}

const EVENT_HANDLERS = {
  "payment.captured": handlePaymentCaptured,
  // CashFree can deliver either event as the successful-order notification.
  // payment.captured carries the payment entity; order.paid may only carry
  // the order, so resolve its captured payment from CashFree when needed.
  "order.paid": async (payload) => {
    ...
    if (!orderId) return;
    const payments = await CashFreeService.fetchOrderPayments(orderId);
...
// needs the exact original bytes CashFree signed, not a re-serialized
...
export const handleCashFreeWebhook = asyncHandler(async (req, res) => {
  ...
  // Idempotency — a duplicate delivery of an already-seen event (CashFree
  ...
  try {
    ...
    // failure, so CashFree doesn't enter a retry storm. processing_error
    // keeps the durable raw payload queryable/replayable by staff.
    console.error(`[CashFree webhook] ${eventType} (${eventId}) failed:`, err);
