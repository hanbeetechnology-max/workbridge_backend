-- Real CashFree Route integration — replaces the simulated flat-15%-hidden
...
-- those directly; WorkBridge only ever persists the acc_XXXX id CashFree
-- hands back, never the raw bank details themselves.
ALTER TABLE users ADD COLUMN CashFree_account_id     TEXT UNIQUE;
ALTER TABLE users ADD COLUMN CashFree_account_status CashFree_account_status NOT NULL DEFAULT 'NOT_LINKED';
ALTER TABLE users ADD COLUMN CashFree_account_email  TEXT;
ALTER TABLE users ADD COLUMN CashFree_linked_at      TIMESTAMPTZ;
...
ALTER TABLE projects ADD COLUMN funding_method   TEXT NOT NULL DEFAULT 'CashFree'
  CHECK (funding_method IN ('CashFree', 'MANUAL_BANK_TRANSFER'));
ALTER TABLE projects ADD COLUMN CashFree_order_id    TEXT UNIQUE;
ALTER TABLE projects ADD COLUMN CashFree_payment_id  TEXT;
ALTER TABLE projects ADD COLUMN CashFree_transfer_id TEXT;
ALTER TABLE projects ADD COLUMN CashFree_refund_id   TEXT;
...
-- ACTIVE CashFree account at completion time.
ALTER TABLE transactions ADD COLUMN settlement_method TEXT NOT NULL DEFAULT 'WALLET'
  CHECK (settlement_method IN ('WALLET', 'CashFree_ROUTE_AUTO', 'WALLET_PENDING_MANUAL'));
